...
# Turns (B, C, T) → (B, D)
# ---------------------------------------------------------------------------
class TCNModel_v1_outch64_GELU_head2_small_v6(nn.Module): 
    def __init__(self, in_channels=8, hidden=16, D=128):
        super().__init__()
        ...
        for d in [1, 2, 4, 8, 16, 32]:
            layers += [
                    nn.Conv1d(in_ch, out_ch, 3, padding=d, dilation=d), 
                       nn.BatchNorm1d(out_ch), 
                    nn.GELU()
                    ]
            in_ch = out_ch
...
# It is a lightweight MLP (2-layer) as recommended in the TRM paper.
# ---------------------------------------------------------------------------
class TRMSharedNet_v6(nn.Module):
    def __init__(self, D):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(D, D),
...
class TRM_EEG_Model_v6_1(BaseModel_new):
    def __init__(self, 
        ...
        # 1) EEG encoder: (B, C, T) → (B, D)
        self.encoder = TCNModel_v1_outch64_GELU_head2_small_v6(in_channels=in_channels, D=D)
        ...
        # 3) shared TRM tiny network
        self.shared_net = TRMSharedNet_v6(D)
