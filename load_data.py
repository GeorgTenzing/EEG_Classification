...
def convert_data_biowolf_GUI_2c(file_including_path, VoltageScale, TimeStampScale):
    ...
    with open(file_including_path, 'rb') as fileID:
        A = np.frombuffer(fileID.read(), dtype=np.uint8)
        # A = np.frombuffer(fileID.read(), dtype=np.uint64)
    ...
    ADS_32 = ADS.astype(np.uint32)

    # Each 24-bit channel is encoded as 3 bytes (big endian)
    def decode_24bit(ch_bytes):
        return (ADS_32[:, ch_bytes[0]] << 16) | (ADS_32[:, ch_bytes[1]] << 8) | ADS_32[:, ch_bytes[2]]

    ch11 = decode_24bit([0, 1, 2])
    ch22 = decode_24bit([3, 4, 5])
    ch33 = decode_24bit([6, 7, 8])
    ch44 = decode_24bit([9, 10, 11])
    ch55 = decode_24bit([12, 13, 14])
    ch66 = decode_24bit([15, 16, 17])
    ch77 = decode_24bit([18, 19, 20])
    ch88 = decode_24bit([21, 22, 23])
    ### ----------------------------------------------------------------------
    ...
    # --- Helper to decode 16- or 24-bit acceleration channels ---
    def decode_acc(ch_bytes):
        # combine two bytes into one 16-bit value (high-byte first)
        return (ADS_32[:, ch_bytes[0]] << 8) | ADS_32[:, ch_bytes[1]]

    # --- Decode accelerometer channels safely ---
    acc_x = decode_acc([25, 26])
    acc_y = decode_acc([27, 28])
    acc_z = decode_acc([29, 30])

    # --- Convert to signed 16-bit if necessary (depends on IMU) ---
    def to_signed16(x):
        x = x.astype(np.int32)
        x[x >= 2**15] -= 2**16
        return x
...
def convert_raw_to_csv(folder_path, time_scale, voltage_scale, save_path):
    ...
    for file in listing:
        ...
        # ExG_data = convert_data_biowolf_GUI_2c_new(file_path, voltage_scale, time_scale)
